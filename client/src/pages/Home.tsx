...
import { CONTENT_STORAGE_KEY, defaultContent, type PropertyKind, type SiteContent } from "@/data/content";

const filters: Array<"전체" | PropertyKind> = ["전체", "토지", "전원주택", "농지"];

const getContent = (): SiteContent => {
  try {
...
export default function Home() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    toast.success("문의 내용이 접수 화면에 저장되었습니다.", { description: "실제 운영 전에는 문자, 이메일 또는 상담 관리 도구와 연결해 주세요." });
  ...
  return (
    <div className="site-shell">
      <header className="site-header">
        ...
        <nav className="main-nav" aria-label="주요 메뉴"><a href="#properties">매물 보기</a><a href="#journal">청양 노트</a><Link href="/manage">운영 미리보기</Link></nav>
        <a className="nav-call" href={phoneHref}><Phone size={15} /> 전화 상담</a>
      </header>

      <main id="top">
        <section className="hero-section">
          <div className="hero-image" aria-hidden="true"><img src="/manus-storage/cheongyang-hero-ridge_a94ba82e.jpg" alt="" /></div>
        ...
        <section id="properties" className="property-section">
          <div className="section-heading"><div><p className="eyebrow">FEATURED FIELD NOTES</p><h2>이번 주, 현장에서<br />다시 보는 조건들</h2></div><p>아래 매물은 디자인 검토를 위한 예시입니다.<br />실제 운영 시 매물명·사진·가격을 교체해 주세요.</p></div>
          ...
          <div className="property-grid">
            {visibleProperties.map((property, index) => (
              <article className={`property-card property-${property.tint} ${!property.image ? "is-data-card" : ""}`} key={property.id}>
                {property.image ? <div className="property-image"><img src={property.image} alt="" /><span>DEMO LISTING</span></div> : <div className="data-card-art"><Sprout size={52} strokeWidth={1} /><span>FIELD<br />CHECK</span></div>}
        ...
        <section id="journal" className="journal-section">
          <div className="journal-heading"><div><p className="eyebrow">CHEONGYANG JOURNAL</p><h2>청양을 알고<br />고르는 일</h2></div><Link href="/manage" className="journal-manage"><FilePenLine size={17} /><span>글처럼 바꾸는<br />운영 화면 보기</span><ArrowRight size={16} /></Link></div>
          <div className="journal-grid">{content.journal.map((entry, index) => <article className={`journal-card journal-card-${index + 1}`} key={entry.id}><div className="journal-card-top"><span>{entry.category}</span><span>FIELD {String(index + 1).padStart(2, "0")}</span></div><h3>{entry.title}</h3><p>{entry.excerpt}</p><div><span>{entry.date} · {entry.readTime}</span><a href="#inquiry" aria-label={`${entry.title} 관련 상담`}>읽어보기 <ArrowRight size={15} /></a></div></article>)}</div>
        </section>

        <section id="inquiry" className="inquiry-section">
          <div className="inquiry-rail"><span>04</span><i /></div>
          <div className="inquiry-intro"><p className="eyebrow">LET'S START WITH A NOTE</p><h2>현장 확인이 필요한<br />매물이라면, 먼저<br />조건을 남겨 주세요.</h2><p>전화가 편하다면 바로 연결하고, 글로 남기고 싶다면 간단한 조건만 보내 주세요.</p><div className="inquiry-call"><span>전화 상담</span><a href={phoneHref}>{content.phone}<Phone size={18} /></a></div><p className="fine-print">* 현재 디자인 검토용 대표번호입니다. 오픈 전 실제 전화번호로 교체해 주세요.</p></div>
          <form className="inquiry-form" onSubmit={handleSubmit}>
            ...
            <button type="submit" className="submit-button">상담 조건 접수하기 <ArrowRight size={17} /></button><small><CheckCircle2 size={14} /> 입력한 정보는 상담 확인 용도로만 사용됩니다.</small>
          </form>
        </section>
      </main>
      <footer className="site-footer"><div className="footer-brand"><img src="/manus-storage/cheongyang-logo-mark_0d5e9394.png" alt="" /><span>{content.officeName}<small>CHEONGYANG FIELD ESTATE</small></span></div><p>사업자 정보와 소재지, 실제 대표번호는 개설 전 필수로 입력해 주세요.</p><div><Link href="/manage">운영 미리보기</Link><a href="#top">맨 위로 <ArrowDownRight size={15} /></a></div></footer>
